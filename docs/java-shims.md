...
- **AntiSamy really sanitises.** `org.owasp.validator.html.AntiSamy` parses the OWASP policy XML you hand it and applies it — tag actions (`validate`/`filter`/`remove`/`truncate`), per-attribute regexp and literal validation with `onInvalid` semantics, and `style`/`<style>` filtering against `<css-rules>` — over html5ever, with no JVM and no jars. Output was diffed against the real AntiSamy 1.5.3 jar across 77 inputs × the six shipped 1.4.4 policies: **no security-relevant divergence**, and the remainder are cosmetic (see `docs/known-issues.md` §50). The same core is available as the `sanitizeHtml(html, policyPath)` BIF.
...
| Class (and aliases) | Supported methods |
|---|---|
| `org.owasp.validator.html.AntiSamy` | `scan(html, policy)` → `CleanResults`. Real policy-driven sanitisation over a native Rust core (`cfml-sanitize`); the jar-path argument is accepted and ignored. Unknown methods **throw** |
| `org.owasp.validator.html.Policy` | static `getInstance(file)` taking a `java.io.File` (or a path string) to an AntiSamy policy XML; `getDirective(name)`. An unreadable or malformed policy raises `PolicyException` at `getInstance`, as the real library does. Parsed policies are cached, so repeated `getInstance` calls are cheap |
| `org.owasp.validator.html.CleanResults` | `getCleanHtml()`. `getErrorMessages()`/`getNumberOfErrors()` **throw** rather than report zero — we do not track per-change messages, and answering "no errors" would falsely imply nothing was removed |
