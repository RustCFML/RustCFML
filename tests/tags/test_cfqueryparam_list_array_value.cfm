...
// ── Datasource leg. QoQ and the real drivers expand list params in different
//    code, so a green QoQ leg says nothing about the driver path — which is
//    where the PostgreSQL failure below actually lives. SQLite is built in, so
//    this runs everywhere. The canary row's id IS the comma-joined string, so
//    it matches only if the array was (wrongly) bound as one literal.
//    SQLite is built into RustCFML but is not a stock Lucee driver, so the leg
//    self-skips where the class will not load rather than reporting a
//    divergence that is really a missing extension.
dsSqlite = { class: "org.sqlite.JDBC", connectionString: "jdbc:sqlite::memory:" };
dsArr = "";
try {
    queryExecute( "CREATE TABLE IF NOT EXISTS probe_arrlist (id VARCHAR(64))", {}, { datasource: dsSqlite } );
    queryExecute( "DELETE FROM probe_arrlist", {}, { datasource: dsSqlite } );
    for ( seedId in [ "a-1", "a-2", "a-1,a-2" ] ) {
        queryExecute( "INSERT INTO probe_arrlist (id) VALUES (:id)",
            { id = { value = seedId, cfsqltype = "cf_sql_varchar" } }, { datasource: dsSqlite } );
    }
    dsArr = queryExecute( "SELECT id FROM probe_arrlist WHERE id IN (:ids) ORDER BY id",
        { ids = { value = [ "a-1", "a-2" ], cfsqltype = "cf_sql_varchar", list = true } },
        { datasource: dsSqlite } );
} catch ( any e ) {
    dsArr = "";
}
if ( isSimpleValue( dsArr ) ) {
    assertTrue( "datasource array-list leg skipped (no SQLite driver on this engine)", true );
    assertTrue( "datasource canary leg skipped (no SQLite driver on this engine)", true );
} else {
    assert( "datasource: an array value expands to one bind per element", dsArr.recordcount, 2 );
    assert( "datasource: the comma-joined canary row is NOT matched",
            valueList( dsArr.id ), "a-1,a-2" );
}
