...
// false. Lucee walks the value's concrete Java class hierarchy. These assertions
// are the EXACT set Lucee 6 returns (probed live) — note Lucee does NOT accept
// the bare type names "Array"/"Struct"/"Boolean"/"Query" here, only the Java /
// Lucee class + interface names (and "String"/"numeric"). Green on both engines.
// ============================================================

arr = [ 1, 2, 3 ];
assert("array is java.util.List", isInstanceOf(arr, "java.util.List"), true);
assert("array is java.util.Collection", isInstanceOf(arr, "java.util.Collection"), true);
assert("array is java.lang.Iterable", isInstanceOf(arr, "java.lang.Iterable"), true);
assert("array is lucee.runtime.type.ArrayImpl", isInstanceOf(arr, "lucee.runtime.type.ArrayImpl"), true);
assert("array is NOT bare 'Array'", isInstanceOf(arr, "Array"), false);
...
assert("struct is lucee.runtime.type.StructImpl", isInstanceOf(st, "lucee.runtime.type.StructImpl"), true);
assert("struct is NOT bare 'Struct'", isInstanceOf(st, "Struct"), false);
...
assert("string is java.lang.CharSequence", isInstanceOf(s, "java.lang.CharSequence"), true);

b = true;
assert("boolean is java.lang.Boolean", isInstanceOf(b, "java.lang.Boolean"), true);
assert("boolean is NOT bare 'Boolean'", isInstanceOf(b, "Boolean"), false);

n = 42;
assert("integer is numeric", isInstanceOf(n, "numeric"), true);
assert("integer is java.lang.Double (Lucee numerics are Double)", isInstanceOf(n, "java.lang.Double"), true);
assert("integer is NOT java.lang.Integer", isInstanceOf(n, "java.lang.Integer"), false);
